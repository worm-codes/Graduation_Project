...
router.post("/publish", async (req, res) => {
  try {
    ...
    theUserAge =
      ...
      parseInt(theUser.user_date_of_birth.substring(0, 4).toString());

    console.log("Inside try block of /publish");

    const theAd = await new Ad({
      arriving_date_year: arrivingDateYear,
      arriving_date_month: arrivingDateMonth,
      arriving_date_day: arrivingDateDay,
      city: city,
      country,
      description,
      host,
      leaving_date_year: leavingDateYear,
      leaving_date_month: leavingDateMonth,
      leaving_date_day: leavingDateDay,
      maxPeople: maxPeopleToPass,
      minTimeHour: minTimeHourToPass,
      maxTimeHour: maxTimeHourToPass,
      minTimeMinute: minTimeMinuteToPass,
      maxTimeMinute: maxTimeMinuteToPass,
      state,
      owner_gender: theUser.user_gender,
      owner_email: theUser.user_email,
      owner_age: theUserAge,
      owner_id: theUser._id,
      minTimeOfAd: minTimeTotal,
      maxTimeOfAd: maxTimeTotal,
    });
  ...
  try {
    ...
    if (minTimeHourToPass !== null && arrivingDateDay !== null) {
      theAds = await Ad.find({
        $and: [
          {
            owner_age: { $gte: minAge },
            owner_age: { $lte: maxAge },
            arriving_date_day: { $gte: arrivingDateDay },
            minTimeOfAd: { $gte: minTimeTotal },
            maxTimeOfAd: { $lte: maxTimeTotal },
            leaving_date_day: { $lte: leavingDateDay },
          },
        ],
        arriving_date_year: arrivingDateYear,
        arriving_date_month: arrivingDateMonth,
    ...
    if (arrivingDateDay === null && minTimeHourToPass === null) {
      theAds = await Ad.find({
        $and: [{ owner_age: { $gte: minAge }, owner_age: { $lte: maxAge } }],
        city: city,
        state: state,
        country: country,
        host: host,
...
router.get("/searchresult", async (req, res) => {
  let ads = store.get("advertisements");
  if (ads === undefined) {
    return res.json("undefined string");
  }
  let searchedAds = JSON.parse(ads);
...
router.post("/deneme", async (req, res) => {
  let returningArr = [{ name: "burak", age: 25 }];
  res.json(returningArr);
});
...
router.delete("/mypastads/:adid", MiddleWare.isAuth, async (req, res) => {
  try {
    ...
    if (adToBeDeleted.isDateActive()) {
      let adToBeDeletedPopulatedAppliedUsers = await adToBeDeleted.populate(
        "appliedUsers"
      );
      let adToBeDeletedPopulatedAcceptedUsers =
        await adToBeDeletedPopulatedAppliedUsers.populate("acceptedUsers");
      let adToBeDeletedPopulatedBannedUsers =
        await adToBeDeletedPopulatedAcceptedUsers.populate("bannedUsers");

      console.log("adToBeDeleted var:", adToBeDeleted);
      console.log(
        "adToBeDeletedPopulatedBannedUsers var:",
        adToBeDeletedPopulatedBannedUsers
      );
      let adToBeDeletedsAcceptedUsers = [];
      for (
        let i = 0;
        i < adToBeDeletedPopulatedBannedUsers.acceptedUsers.length;
        i++
      ) {
        adToBeDeletedsAcceptedUsers.push(
          await User.findById(
            adToBeDeletedPopulatedBannedUsers.acceptedUsers[i]._id
          )
        );
      }
      ...
      for (let i = 0; i < adToBeDeletedsAcceptedUsers.length; i++) {
        await User.findByIdAndUpdate(
          { _id: adToBeDeletedsAcceptedUsers[i]._id },
          { $pull: { acceptedAds: adid } }
        );
      }
      ...
      for (
        let i = 0;
        i < adToBeDeletedPopulatedBannedUsers.appliedUsers.length;
        i++
      ) {
        adToBeDeletedsAppliedUsers.push(
          await User.findById(
            adToBeDeletedPopulatedBannedUsers.appliedUsers[i]._id
          )
        );
      }
      ...
      for (let i = 0; i < adToBeDeletedsAppliedUsers.length; i++) {
        await User.findByIdAndUpdate(
          { _id: adToBeDeletedsAppliedUsers[i]._id },
          { $pull: { appliedAds: adid } }
        );
      }
      ...
      for (
        let i = 0;
        i < adToBeDeletedPopulatedBannedUsers.bannedUsers.length;
        i++
      ) {
        adToBeDeletedsBannedUsers.push(
          await User.findById(
            adToBeDeletedPopulatedBannedUsers.bannedUsers[i]._id
          )
        );
      }
      ...
      for (let i = 0; i < adToBeDeletedsBannedUsers.length; i++) {
        await User.findByIdAndUpdate(
          { _id: adToBeDeletedsBannedUsers[i]._id },
          { $pull: { bannedAds: adid } }
        );
      }
      ...
      await Ad.findByIdAndUpdate(
        { _id: adToBeDeleted._id },
        { $pull: { acceptedUsers: { $exists: true } } }
      );
      await Ad.findByIdAndUpdate(
        { _id: adToBeDeleted._id },
        { $pull: { appliedUsers: { $exists: true } } }
      );
      await Ad.findByIdAndUpdate(
        { _id: adToBeDeleted._id },
        { $pull: { bannedUsers: { $exists: true } } }
      );
...
router.post("/searchresult/:adid/:userid", async (req, res) => {
  ...
  if (
    theFoundAd.appliedUsers.length <= theFoundAd.maxPeople &&
    !theFoundAd.bannedUsers.includes(userid) &&
    !theFoundAd.appliedUsers.includes(userid) &&
    !appliedUser.appliedAds.includes(adid)
  ) {
...
router.put("/searchresult/:adid/:userid/accept", async (req, res) => {
  ...
  if (
    theFoundAd.acceptedUsers.length <= theFoundAd.maxPeople &&
    !theFoundAd.bannedUsers.includes(userid) &&
    !theFoundAd.acceptedUsers.includes(userid) &&
    !appliedUser.acceptedAds.includes(adid) &&
    theFoundAd.appliedUsers.includes(userid) &&
    appliedUser.appliedAds.includes(adid)
  ) {
    await User.findByIdAndUpdate(
    ...
    await Ad.findByIdAndUpdate(
      { _id: theFoundAd._id },
      { $pull: { appliedUsers: appliedUser._id } }
    );
    await appliedUser.save();
    await theFoundAd.save();
    ...
    let populatedAcceptedUser = await initialPopulatedAcceptedUser.populate(
      "acceptedAds"
    );
    ...
    let lastRealFinalAdArrToPass = await realFinalAdArrToPass.populate(
      "acceptedUsers"
    );
    let theTrueLastAdArrToPass = await lastRealFinalAdArrToPass.populate(
      "bannedUsers"
    );
...
router.put("/searchresult/:adid/:userid/decline", async (req, res) => {
  ...
  if (!theFoundAd.bannedUsers.includes(userid)) {
    if (
      ...
      let initialPopulatedAcceptedUser = await appliedUser.populate(
        "appliedAds"
      );
      let initialFoundAd = await theFoundAd.populate("appliedUsers");
      let populatedAcceptedUser = await initialPopulatedAcceptedUser.populate(
      ...
      let finalAdArrToPass = await Ad.findById(adid).populate("acceptedUsers");
      let realFinalAdArrToPass = await finalAdArrToPass.populate(
        "appliedUsers"
      );
      let lastRealFinalAdArrToPass = await realFinalAdArrToPass.populate(
        "acceptedUsers"
      ...
      await Ad.findByIdAndUpdate(
        { _id: theFoundAd._id },
        { $pull: { acceptedUsers: appliedUser._id } }
      );
      await appliedUser.save();
      await theFoundAd.save();
