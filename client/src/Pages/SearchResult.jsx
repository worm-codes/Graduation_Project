...
const SearchResult = () => {
  ...
  useEffect(() => {
    const getFilteredAds = async () => {
      const response = await axios.get(
        `http://localhost:5000/api/ad/searchresult`,
        {
          headers: {
            Authorization:
  ...
  let filteredCities = City.getCitiesOfState(
    selectedStatesCountryCode,
    selectedStatesIsoCode
  ).filter((city) =>
          ...
          >
            ...
            <div className="col">
              ...
              !isStateValidForCountry ? (
                <p style={{ color: "red" }}>State does not belong to country</p>
              ) : (
                ""
              )}
            ...
            <div className="col">
              ...
              cityVar.name &&
              !isCityValidForState ? (
                <p style={{ color: "red" }}>City does not belong to state</p>
              ) : (
                ""
              )}
                ...
                <div className="query-content">
                  <div
                    onClick={async (e) => {
                      e.preventDefault();
                      const response = await axios.post(
                        `http://localhost:5000/api/ad/searchresult/${ad._id}`,
                        {}
                      );
                      console.log(response.data);
                  ...
                  <div className="conditionalFlex">
                    <div className="query-text">
                      ...
                      <p id="bigscreentext">
                        ...
                        elit. Quaerat impedit ad accusamus provident libero
                        animi neque labore, iusto maiores odit. Veniam dolor non
                        reprehenderit necessitatibus debitis architecto repellat
                        ratione ullam. Repellendus quisquam voluptatum
                        accusantium, debitis molestias accusamus libero sunt
                        expedita minus aliquam pariatur molestiae voluptatem
                        aperiam doloribus ullam ducimus consectetur! Quod
                        laudantium inventore modi molestiae nesciunt, cupiditate
                        numquam quasi. Odio?
                      </p>

                      <p id="mediumscreentext">
                        Lorem ipsum dolor sit amet consectetur adipisicing elit.
                        Eligendi autem reiciendis, reprehenderit quae fugit sed
                        excepturi cumque sequi odio voluptate omnis hic
                        accusantium natus minus magni, doloremque, ipsa ab cum?
                      </p>
                      <p id="smallscreenemptytext"></p>
                    </div>
                    <div
                      style={{ justifyContent: "space-between" }}
                      className="dateAndTimeDiv"
                    >
                      <p>{ad.city}</p>
                      <p>{`Date: ${ad.arriving_date_day.toString()} ${
                        months[ad.arriving_date_month - 1]
                      } - ${ad.leaving_date_day.toString()} ${
                        months[ad.leaving_date_month - 1]
                      }`}</p>
                      <p
                        style={{ marginRight: "3em" }}
                        id="timeText"
                      >{`From:  ${decideToPutZero(
                        ad.minTimeHour
                      )}:${decideToPutZero(
                        ad.minTimeMinute
                      )} - To: ${decideToPutZero(
                        ad.maxTimeHour
                      )}:${decideToPutZero(ad.maxTimeMinute)}`}</p>
                    </div>
                  </div>
                </div>
              ))
