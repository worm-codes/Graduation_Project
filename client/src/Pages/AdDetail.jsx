...
const getCurrentUserInfo=async()=>{   
 ...
 let isAdActive = () => {
  ...
  if(theRealAd?.arriving_date_year === currentYear && theRealAd?.arriving_date_month > currentMonth){
      return true;
  }

  if(theRealAd?.arriving_date_year === currentYear && theRealAd?.arriving_date_month === currentMonth && theRealAd?.arriving_date_day > currentDay){
      return true;
  }
  ...
  if(theRealAd?.arriving_date_year === currentYear && theRealAd?.arriving_date_month === currentMonth && theRealAd?.arriving_date_day === currentDay && 
      theRealAd?.minTimeHour === currentHour && theRealAd?.minTimeMinute > currentMinute) {
          return true;
      }
...
useEffect(() => {
  if(isAdActive()){
    ...
    console.log("updatedAdvertisement in useEffect else if:", updatedAdvertisement);
      ...
      onClick={()=>{navigate('/profile/'+acceptedUsr?._id)}}
        ...
        {loggedInUser?._id === theAdOwner?._id ?
          <a onClick={async(e) => {e.preventDefault();
            const response = await axios.put(`http://localhost:5000/api/ad/searchresult/${theRealAd?._id}/${acceptedUsr._id}/decline`, {
            });
...
let currentlyUnacceptedAppliedUsers = theRealAd?.appliedUsers?.filter((usr) => !theRealAd?.acceptedUsers.includes(usr))
...
let appliedUserRenderFunc = theRealAd?.appliedUsers?.map((appliedUsr) => {
  ...
  return ( valueOfIsAdActive ?
   ...
   alt="avatar image"/>
           ...
           <div>
           <a onClick={async(e) => {e.preventDefault();
            const response = await axios.put(`http://localhost:5000/api/ad/searchresult/${theRealAd?._id}/${appliedUsr._id}/decline`, {
            });
            response.data !== "User has already been rejected for this ad." ?     
                  setTheRealAd(response.data.foundAd) &&
                  setTheAdOwner(response.data.adOwner)
...
let decidingOfAppliedUserRenderFunc = loggedInUser?._id === theAdOwner?._id && theRealAd?.appliedUsers?.length > 0 && valueOfIsAdActive;
     ...
     <div  className='outerContainer'>
      ...
      {decidingOfAppliedUserRenderFunc ?
        ...
        <div className='acceptedUserContainer'>
         ...
         <img    onClick={()=>{navigate('/profile/'+theAdOwner?._id)}}
				src="https://www.kindpng.com/picc/m/22-223941_transparent-avatar-png-male-avatar-icon-transparent-png.png"
				className="rounded-circle z-depth-0 avatar OwnerAvatar"
				alt="avatar image"/>
                <div className='sideBarContent'>
                <Link to={`/profile/${theAdOwner?._id}`} style={{textDecoration:'none'}} className="sideBar-title h2">{theAdOwner?.user_ID}</Link>
                ...
                {loggedInUser?._id === theAdOwner?._id && valueOfIsAdActive ?
                  <div onClick={async(e) => {e.preventDefault();
                    const response = await axios.delete(`http://localhost:5000/api/ad/mypastads/${theRealAd?._id}`, {
                      headers:{Authorization: 'Bearer ' + await useAuth.currentUser.getIdToken(true)}
                    }) 
                ...
                {loggedInUser?._id !== theAdOwner?._id && applyButtonDisplayCondition ?  
                  <div onClick={async(e) => {e.preventDefault();
                    const response = await axios.post(`http://localhost:5000/api/ad/searchresult/${theRealAd?._id}/${loggedInUser._id}`, {
                  });
