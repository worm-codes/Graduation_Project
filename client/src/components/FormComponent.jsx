...
const [stateVar, setStateVar] = useState([])
  ...
  let host = watch().props?.publishHost?.props?.children[1]?.props?.name ? watch().props?.publishHost?.props?.children[1]?.props?.name : '';
...
// console.log("description", handleDescText);
  ...
  let dateToCheck = new Date();
    ...
    //useEffect's to set the countryVar, stateVar and cityVar state variables 
      ...
      let urlToSendRequestTo = currentPath === '/publish' ? 'publish' : 'searchresult'
      let hostOptionToSend = currentPath === '/publish' ?  props.publishHostVal : props.searchHostVal
          ...
          <div className="containerr">
            ...
            <form
              onSubmit={handleSubmit(async (data,event) => {
                ...
                const response = await axios.post(`http://localhost:5000/api/ad/${urlToSendRequestTo}`, {
                ...
                //  console.log("data sended via /publish page:",response);
                {currentPath === '/publish' ? window.location.assign(`/searchresult/${response?.data?._id}`) :
              ...
              <div className="yusuf-container">
                ...
                <div className="roww">
                  ...
                  <div className="columnn">
                    ...
                    <input
                      ...
                      {...currentPath === '/publish' ? register("arriving", {required:'Please select an arrival date'}) : ''}
                      name="arriving"
                      id="arriving"
                      type="date"
                    />
                    {currentPath === '/publish' && errors?.arriving?.message ? ((!errors.country && !errors.state && !errors.city && isFoundCountry && isFoundState && cityInput) ? <p style={{color:'red'}}>{errors.arriving.message}</p> : '') : ''}
                  </div>
  
                  <div className="columnn">
                    <label htmlFor="leaving">Leaving in:</label>
                    <input min={`${arrivalDate}`}  disabled={!isArrivingDateSelected}
                    {...currentPath === '/publish' ? register("leaving", {required:'Please select a leaving date'}) : ''}
                ...
                <div className="roww">
                  <div className="columnn">
                    ...
                    {...currentPath === '/publish' ? register("maxPeople", {required:'Please select maximum people to guide'}) : ''}>
                  ...
                  <div className="columnn" id="minTime">
                    ...
                    <input
                      ...
                      {...currentPath === '/publish' ? register("minTime", {required:'Please select starting time'}) : ''}
                      name="minTime"
                      id="minTime"
                      type="time"
                    />
                  ...
                  <div className="columnn" id="maxTime">
                    ...
                    <input
                     ...
                     {...currentPath === '/publish' ? register("maxTime", {required:'Please select ending time'}) : ''}
                      name="maxTime"
                       id="maxTime"
                        type="time"
                         />
